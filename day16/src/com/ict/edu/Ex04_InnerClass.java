...
public class Ex04_InnerClass {
	...
	//  특징) 내부클래스에서는 외부클래스의 멤버들을 마음대로 사용 가능(상속비슷)
	// 		단, 상속에서는 private 접근 안됨, 내부클래스에서는 private 접근 가능
	//		외부클래스를 통해서만 내부클래스를 생성할 수 있다.(내부클래스만 별도로 생성 안됨)
	
	// 2. Local : 외부 클래스 메소드 안에 내부 클래스가 존재
	//			  외부클래스가 지역 변수처럼 사용
	//			  외부클래스의 메소드가 호출(실행)될때 생성 할 수 있다.
	//			  지역변수처럼 외부클래스의 해당 메소드가 종료되면 내부클래스도 지워진다.
	//		외부클래스{
	//			외부멤버필드
	//			생성자
	//			외부멤버메서드{
	...
	//						내부클래스의 멤버변수는 static을 사용할 수 없다.
	//						어쩔수 없이 내부 클래스의 멤버변수를 static으로 사용할때
